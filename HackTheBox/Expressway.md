...
- Intially i tried running tcp port scan, and I didn't find anything except for port 22 ssh.
```
...
```
- **SA=(Enc=3DES Hash=SHA1 Group=2:modp1024 Auth=PSK LifeType=Seconds LifeDuration=28800)**. This line tells the connection needs a PSK for authentication and uses 3DES for encryption and SHA1 for integrity check.

- The IPSec connection establishment was carried out using 2 phases.
- phase1: IKE tunnel establishment (Two modes MAIN MODE and AGGRESSIVE MODE)
- phase2: IPSec tunnel establishment 

- In the phase1, if the server uses AGGRESSIVE MODE, there may be a chance that it leaks some sensitive info about the handshake. (cause it uses 3 Messages)

- The leaks may contain user identity or PSK.
...
- In the comand the **-A** option specifies  aggressive mode, and **-P** option looks for any PSK leaks and save it to psk.txt

- From the output, we can see the use of FQDN ike@expressway.htb.
- And also the PSK is saved in psk.txt
...
- Let's crack the hash for that. We need to capture the full handshake for the Identity ike@expressway.htb
...
- The above command saves the PSK to hash.txt. Now we can start our offline cracking.
...
- Let's try to log in via ssh with username ike
...
- In the ike user home directory, we can find users.txt

**users.txt: 2b3b2313c8133ff8b2e20c9eb2abc323**

## Root.txt

- I downloaded linpeas.sh and ran it. found a few interesting things.
...
```
- found an internal IP 192.168.68.50

```
1753229688.902      0 192.168.68.50 TCP_DENIED/403 3807 GET http://offramp.expressway.htb - HIER_NONE/- text/html
```
- In this specific entry, we can see a subdomain. lets see what in here.
- From the logs, the user tries to access the offramp subdomain, but it is an internal-only site he got a 403. 
...
- But I don't have solid proof. We don't have permission to see the /etc/sudoers file. But from the clues, there is a high likelihood that this attack vector may work.

- I ran the following command, and it dropped me into a root shell.
